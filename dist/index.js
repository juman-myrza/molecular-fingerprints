...
class MolecularFingerprint {
    ...
    static getNeighbors(atomIndex, bonds, atoms, radius) {
        const neighbors = [];
        const visited = new Set();
        const queue = [[atomIndex, 0]];
        while (queue.length > 0) {
            const [currentAtomIndex, currentRadius] = queue.shift();
            if (currentRadius > radius)
                break; // stop if reached the specified radius
            if (visited.has(currentAtomIndex))
                continue;
            visited.add(currentAtomIndex);
            if (currentRadius > 0)
                neighbors.push(currentAtomIndex);
    ...
    static generateSubstructureHash(atomIndex, atoms, bonds, radius) {
     ...
     * @returns fingerprint
     */
    static convertToMorganFingerprint(smiles, radius = MolecularFingerprint.RADIUS) {
        const atoms = [];
        const bonds = [];
        ...
        const fingerprint = Array(MolecularFingerprint.BIT_VECTOR_SIZE).fill(0);
...
MolecularFingerprint.BIT_VECTOR_SIZE = 1024;
MolecularFingerprint.RADIUS = 2;
// prettier-ignore
MolecularFingerprint.ATOM_SYMBOLS = [
...
MolecularFingerprint.BOND_TYPES = ['-', '=', '#', ':', '/', '\\'];
MolecularFingerprint.BASE_PRIME = 67;
